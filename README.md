###微信小程序加密数据解密算法Go版 by Jokki

**微信小程序官方说明文档签名加密篇链接**
> https://developers.weixin.qq.com/miniprogram/dev/api/signature.html

**使用方法**

> go get github.com/jokkijiang/wxGoStore

**引入方法**
```Go
import (
	"github.com/jokkijiang/wxGoStore"
)
```
**使用示例**

```Go
package main

import (
	"fmt"
	"github.com/jokkijiang/wxGoStore"
)

func main() {
	appID := "wx4f4bc4dec97d474b"
	sessionKey := "tiihtNczf5v6AKRyjwEUhQ=="
	encryptedData := ""
	iv := "r7BXXKkLb8qrSNn05n0qiA=="

	pc := wxGoStore.WxDataCrypt{AppID: appID, SessionKey: sessionKey}
	result, err := pc.Decrypt(encryptedData, iv, true) //第三个参数解释： 需要返回 JSON 数据类型时 使用 true, 需要返回 map 数据类型时 使用 false
	if err != nil {
		fmt.Println(err)
	} else {
		fmt.Println(result)
	}
}
```


